...
class BaseRiskScore:
    ...
    def _low_risk(self, score: float) -> float:
        """Computes the membership degree for low risk based on churn probability."""
        return max(0, min(1, (0.3 - score) / 0.3))

    def _medium_risk(self, score: float) -> float:
        """Computes the membership degree for medium risk based on churn probability."""
        if score <= 0.2:
            return 0
        elif score <= 0.45:
    ...
    def _high_risk(self, score: float) -> float:
        """Computes the membership degree for high risk based on churn probability."""
        return max(0, min(1, (score - 0.6) / 0.15))

    def fit(
        self, probabilities: np.ndarray[Any, Any] | float | int | list[float]
    ) -> list[int]:
        """Determines the risk level for one or more customers based on their churn probabilities."""
