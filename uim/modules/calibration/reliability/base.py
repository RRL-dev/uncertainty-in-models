...
class CalibrationReliability:
    ...
    def compute_calibration(
        ...
        avg_accuracy: np.floating[Any] = np.average(
            a=stat_bins["bin_accuracies"], weights=stat_bins["bin_counts"]
        )

        avg_confidence: np.floating[Any] = np.average(
            a=stat_bins["bin_confidences"], weights=stat_bins["bin_counts"]
        )

        ece: np.float64 = np.sum(
            a=stat_bins["bin_counts"]
            * np.abs(stat_bins["bin_accuracies"] - stat_bins["bin_confidences"])
        ) / np.sum(a=stat_bins["bin_counts"])

        mce: np.float64 = np.max(
            a=np.abs(stat_bins["bin_accuracies"] - stat_bins["bin_confidences"])
        )
