...
def binary_binned_statistics(
    ...
    digitized: np.ndarray[Any, np.dtype[np.signedinteger[Any]]] = np.searchsorted(
        a=bins[1:-1], v=confidences
    )
    bin_counts: np.ndarray[Any, np.dtype[np.signedinteger[Any]]] = np.bincount(
        digitized, minlength=len(bins)
    )

    bin_acc: np.ndarray[Any, np.dtype[np.signedinteger[Any]]] = np.bincount(
        digitized, weights=(targets == pred_labels), minlength=len(bins)
    )
    bin_prob: np.ndarray[Any, np.dtype[np.signedinteger[Any]]] = np.bincount(
        digitized, weights=confidences, minlength=len(bins)
    )
    bin_true: np.ndarray[Any, np.dtype[np.signedinteger[Any]]] = np.bincount(
        digitized, weights=targets, minlength=len(bins)
    )
